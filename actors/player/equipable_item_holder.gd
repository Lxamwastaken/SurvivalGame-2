...
func equip_item(item_key : ItemConfig.Keys) -> void:
	...
	var item_scene := ItemConfig.get_equipable_item(item_key).instantiate()
	
	
	if current_item_scene is EquipableWeapon:
		current_item_scene.weapon_item_resource = ItemConfig.get_item_resource(item_key)

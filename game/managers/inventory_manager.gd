...
func switch_two_item_indexes(idx1 : int, indx1_is_in_hotbar : bool, idx2 : int, indx2_is_in_hotbar : bool) -> void:
	var item1 = inventory[idx1] if not indx1_is_in_hotbar else hotbar[idx1]
	var item2 = inventory[idx2] if not indx2_is_in_hotbar else hotbar[idx2]
	
	if not indx1_is_in_hotbar:
		inventory[idx1] = item2
	else:
		hotbar[idx1] = item2
	
	if not indx2_is_in_hotbar:
